...
bool FT6X36::begin(uint8_t threshold)
{
  _wire->begin();
	...
	//writeRegister8(FT6X36_REG_DEVICE_MODE, 0x04); // Factory/Settings mode
 	//writeRegister8(FT6X36_REG_DEVICE_MODE, 0x00); // working mode
	//writeRegister8(FT6X36_REG_THRESHHOLD, threshold);
  //writeRegister8(FT6X36_REG_CTRL, 0x00); // Always active mode
  writeRegister8(FT6X36_REG_TOUCHRATE_ACTIVE, 10); // Report rate in MS
 	writeRegister8(FT6X36_REG_DEVICE_MODE, 0x00); // working mode

  Serial.println("FT6x36 connected - All good!");
	return true;
}
